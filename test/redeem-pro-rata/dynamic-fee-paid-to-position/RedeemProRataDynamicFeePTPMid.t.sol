...
contract RedeemProRataDynamicFeePTPMidTest is RedeemProRataBaseTest {

    function setUp() public virtual override {
        systemParams.redemptionBaseFee = 0.005 ether;       // 0.5% base fee
        systemParams.redemptionDynamicFeeScalar = 1 ether;
        systemParams.redemptionFeeDecayPeriod = 0;          // no decay
    ...
    function _getExpectedFeePercentage(uint256 redemptionAmount) internal override view returns (uint256) {
        uint256 feePercentage = nectra.getRedemptionFee(redemptionAmount);
        return feePercentage > UNIT ? UNIT : feePercentage;
    }
    ...
    function test_redeem_ExceedBuffer_ProRataBetweenBucketsBelow() public {
        ...
        expectedCollaterals[0] = collateralsBefore[0] - 3 * splitAmount * (UNIT - redemptionFeePercentage) / price;
    ...
    function test_redeem_ExceedBuffer_ProRataBetweenAtAndAbove() public {
        ...
        expectedCollaterals[2] = collateralsBefore[2] - splitAmount * (UNIT - secondRedemptionFeePercentage) / price;
        expectedCollaterals[3] = collateralsBefore[3] - splitAmount * (UNIT - secondRedemptionFeePercentage) / price;
        expectedCollaterals[4] = collateralsBefore[4] - splitAmount * (UNIT - secondRedemptionFeePercentage) / price;
